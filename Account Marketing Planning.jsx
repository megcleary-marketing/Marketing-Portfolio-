...
const BP1=`You are analyzing a B2B target account. Return ONLY these labeled lines. No JSON. No markdown. No extra text. One short sentence per value.
...
PIPELINE: [0-10]
TIER: [A, B, or C]
RATIONALE: [one sentence explaining tier]
WHY: [one sentence — why this account, why now]
GTM: [GTM motion name]
MANGLE: [one sentence — marketing angle]
SANGLE: [one sentence — sales angle]
OVERVIEW: [one sentence — company overview]
INDUSTRY: [industry name]
REVENUE: [revenue estimate]
EMPLOYEES: [employee count estimate]
STATUS: [Public or Private]
GEO: [geographic footprint]
INIT1: [recent initiative — one sentence]
INIT2: [recent initiative — one sentence]
INIT3: [recent initiative — one sentence]`;

const BP2=`You are analyzing a B2B target account. Return ONLY these labeled lines. No JSON. No markdown. No extra text. One short sentence per value.

SIG1TYPE: [signal type label]
SIG1DETAIL: [one sentence detail]
SIG1STR: [Strong, Medium, or Weak]
SIG2TYPE: [signal type label]
SIG2DETAIL: [one sentence detail]
SIG2STR: [Strong, Medium, or Weak]
SIG3TYPE: [signal type label]
SIG3DETAIL: [one sentence detail]
SIG3STR: [Strong, Medium, or Weak]
SIG4TYPE: [signal type label]
SIG4DETAIL: [one sentence detail]
SIG4STR: [Strong, Medium, or Weak]
P1TITLE: [persona title]
P1GOALS: [one sentence]
P1PAIN: [one sentence]
P1MSG: [one sentence]
P1OFFER: [one sentence]
P2TITLE: [persona title]
P2GOALS: [one sentence]
P2PAIN: [one sentence]
P2MSG: [one sentence]
P2OFFER: [one sentence]
P3TITLE: [persona title]
P3GOALS: [one sentence]
P3PAIN: [one sentence]
P3MSG: [one sentence]
P3OFFER: [one sentence]`;

const BP3=`You are analyzing a B2B target account. Return ONLY these labeled lines. No JSON. No markdown. No extra text.

CLOUD1: [vendor name]|[High, Medium, or Low]
CLOUD2: [vendor name]|[High, Medium, or Low]
SEC1: [vendor name]|[High, Medium, or Low]
SEC2: [vendor name]|[High, Medium, or Low]
SEC3: [vendor name]|[High, Medium, or Low]
ID1: [vendor name]|[High, Medium, or Low]
CRM1: [vendor name]|[High, Medium, or Low]
MKTG1: [vendor name]|[High, Medium, or Low]
DATA1: [vendor name]|[High, Medium, or Low]
DATA2: [vendor name]|[High, Medium, or Low]
...
T2CH1: [channel]
T2CH2: [channel]
AVOID1: [channel to avoid]
AVOID2: [channel to avoid]
...
KPI2: [kpi]
KPI3: [kpi]
SDR: [one to two sentences — SDR outreach angle]
ETHEME: [executive email theme]
DQ1: [discovery question]
DQ2: [discovery question]
DQ3: [discovery question]
FOLLOWUP: [follow-up asset name]
OBJ1: [objection — response in one sentence]
OBJ2: [objection — response in one sentence]
HANDOFF: [two to three sentence executive handoff paragraph]`;
...
Write paid media copy using EXACTLY this format:

INTENT1: [intent keyword]
INTENT2: [intent keyword]
INTENT3: [intent keyword]
INTENT4: [intent keyword]
INTENT5: [intent keyword]
SEARCH1: [search keyword]
SEARCH2: [search keyword]
SEARCH3: [search keyword]
BOMBORA1: [Bombora topic]
BOMBORA2: [Bombora topic]
BOMBORA3: [Bombora topic]
G2CAT1: [G2 category]
G2CAT2: [G2 category]
HEADLINE: [ad headline under 55 chars]
BODYCOPY: [one punchy sentence]
CTA: [CTA text]
...
export default function App(){
  ...
  const parseBP1 = (text) => ({
    scores:{
      icpFit: parseFloat(getLine(text,'ICPFIT'))||0,
      intentSignals: parseFloat(getLine(text,'INTENT'))||0,
      expansionPotential: parseFloat(getLine(text,'EXPANSION'))||0,
      strategicValue: parseFloat(getLine(text,'STRATEGIC'))||0,
      pipelinePotential: parseFloat(getLine(text,'PIPELINE'))||0,
      tier: getLine(text,'TIER')||'B',
      tierRationale: getLine(text,'RATIONALE'),
    },
    executiveSummary:{
      whyThisAccount: getLine(text,'WHY'),
      recommendedGTMMotion: getLine(text,'GTM'),
      marketingAngle: getLine(text,'MANGLE'),
      salesAngle: getLine(text,'SANGLE'),
    },
    companyProfile:{
      overview: getLine(text,'OVERVIEW'),
      industry: getLine(text,'INDUSTRY'),
      revenueEstimate: getLine(text,'REVENUE'),
      employeeEstimate: getLine(text,'EMPLOYEES'),
      status: getLine(text,'STATUS'),
      geographicFootprint: getLine(text,'GEO'),
      recentInitiatives: ['INIT1','INIT2','INIT3'].map(k=>getLine(text,k)).filter(Boolean),
    },
  });

  const parseBP2 = (text) => ({
    buyingSignals: [1,2,3,4].map(n=>({
      type: getLine(text,`SIG${n}TYPE`),
      detail: getLine(text,`SIG${n}DETAIL`),
      strength: getLine(text,`SIG${n}STR`)||'Medium',
    })).filter(s=>s.type),
    topPersonas: [1,2,3].map(n=>({
      title: getLine(text,`P${n}TITLE`),
      goals: getLine(text,`P${n}GOALS`),
      painPoints: getLine(text,`P${n}PAIN`),
      messagingAngle: getLine(text,`P${n}MSG`),
      bestContentOffer: getLine(text,`P${n}OFFER`),
    })).filter(p=>p.title),
  });

  const parseBP3 = (text) => {
    const parseTech = (keys) => keys.map(k=>{
      const v=getLine(text,k); if(!v) return null;
      const parts=v.split('|');
      return {name:parts[0]?.trim(),confidence:parts[1]?.trim()||'Medium'};
    }).filter(Boolean);
    return {
      technologyEnvironment:{
        cloudProviders: parseTech(['CLOUD1','CLOUD2']),
        securityVendors: parseTech(['SEC1','SEC2','SEC3']),
        identityVendors: parseTech(['ID1']),
        crm: parseTech(['CRM1']),
        marketingAutomation: parseTech(['MKTG1']),
        dataAnalytics: parseTech(['DATA1','DATA2']),
      },
      campaignRecommendation:{
        primaryMotion: getLine(text,'MOTION'),
        rationale: getLine(text,'RATIONALE'),
        tier1Channels: ['T1CH1','T1CH2','T1CH3'].map(k=>getLine(text,k)).filter(Boolean),
        tier2Channels: ['T2CH1','T2CH2'].map(k=>getLine(text,k)).filter(Boolean),
        channelsToAvoid: ['AVOID1','AVOID2'].map(k=>getLine(text,k)).filter(Boolean),
        campaignTheme: getLine(text,'THEME'),
        contentAsset: getLine(text,'ASSET'),
        suggestedCTA: getLine(text,'CTA'),
        kpis: ['KPI1','KPI2','KPI3'].map(k=>getLine(text,k)).filter(Boolean),
      },
      salesActivation:{
        sdrOutreachAngle: getLine(text,'SDR'),
        executiveEmailTheme: getLine(text,'ETHEME'),
        discoveryQuestions: ['DQ1','DQ2','DQ3'].map(k=>getLine(text,k)).filter(Boolean),
        followUpAsset: getLine(text,'FOLLOWUP'),
        objectionHandling: ['OBJ1','OBJ2'].map(k=>getLine(text,k)).filter(Boolean),
      },
      executiveHandoff: getLine(text,'HANDOFF'),
    };
  };

  const parsePaidMedia = (text) => {
    const lines = {};
    text.split("\n").forEach(line => {
    ...
    return { paidMedia:{
      intentKeywords: ['INTENT1','INTENT2','INTENT3','INTENT4','INTENT5'].map(k=>lines[k]).filter(Boolean),
      searchKeywords: ['SEARCH1','SEARCH2','SEARCH3'].map(k=>lines[k]).filter(Boolean),
      bomboraTopics: ['BOMBORA1','BOMBORA2','BOMBORA3'].map(k=>lines[k]).filter(Boolean),
      g2Categories: ['G2CAT1','G2CAT2'].map(k=>lines[k]).filter(Boolean),
      adHeadline: get('HEADLINE'),
      adBodyCopy: get('BODYCOPY','BODY'),
      cta: get('CTA'),
    }};
  };
    ...
    try{
      ...
      const t1=await callText(BP1,ctx);
      const p1=parseBP1(t1);
      setLoadingStep("Step 2 of 3 — Analyzing signals & personas…");
      const t2=await callText(BP2,ctx);
      const p2=parseBP2(t2);
      setLoadingStep("Step 3 of 3 — Building campaign recommendations…");
      const t3=await callText(BP3,ctx);
      const p3=parseBP3(t3);
      setResult({...p1,...p2,...p3});
  ...
  const generateActivation=async()=>{
    ...
    try{
      ...
      const t3=await callText(AP3(company,solution,result),`Paid media for ${company}`);
      const a3=parsePaidMedia(t3);
